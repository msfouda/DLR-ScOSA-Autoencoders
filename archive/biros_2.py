...
train_dataset = biros_data[:int(0.9*len(biros_data))]
# train_dataset = np.array([train_dataset, train_dataset])
test_dataset = biros_data[int(0.9*len(biros_data)):]
...
def convolutional_auto_encoder():
  ...
  d_c = tf.keras.Model(inputs=bottleneck_output, outputs=decoder_output)
  e_c = tf.keras.Model(inputs=inputs, outputs=bottleneck_output)
  return model, encoder_model, e_c, d_c

convolutional_model, convolutional_encoder_model, e_c, d_c = convolutional_auto_encoder()
...
# save the model
convolutional_model.save('biros_model_mse_256_final_12.h5')
convolutional_encoder_model.save('biros_encoder_model_mse_256_final_12.h5')
e_c.save('biros_enc_mse_256_final_12.h5')
d_c.save('biros_dec_mse_256_final_12.h5')
